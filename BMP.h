...
///Escribe una region en los datos de entrada
    void crear_forma(uint32_t x0, uint32_t y0, uint32_t w, uint32_t h, uint8_t B, uint8_t G, uint8_t R, uint8_t A) {
        ...
        for (uint32_t y = y0; y < y0 + h; ++y) {
            for (uint32_t x = x0; x < x0 + w; ++x) {
                ...
                if (channels == 4) { //para la parte que contiene transparencia :)
    ...
    void draw_rectangle(uint32_t x0, uint32_t y0, uint32_t w, uint32_t h,
        ...
        crear_forma(x0, y0, w, line_w, B, G, R, A);                                             // top line
        crear_forma(x0, (y0 + h - line_w), w, line_w, B, G, R, A);                              // bottom line
        crear_forma((x0 + w - line_w), (y0 + line_w), line_w, (h - (2 * line_w)), B, G, R, A);  // right line
        crear_forma(x0, (y0 + line_w), line_w, (h - (2 * line_w)), B, G, R, A);                 // left line
    }
